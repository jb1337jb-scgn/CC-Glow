...
<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>chargecloud Glow Dashboard</title><style>
:root{--bg:#07051b;--card:rgba(18,15,48,.78);--card2:rgba(8,27,58,.72);--line:rgba(255,255,255,.12);--text:#f8f5ff;--muted:#b9afd8;--pink:#ff3ecf;--cyan:#00d9ff;--blue:#367cff;--green:#28f5a4;--red:#ff4d7d;--yellow:#ffd166}*{box-sizing:border-box}body{margin:0;font-family:Inter,Arial,sans-serif;color:var(--text);background:radial-gradient(circle at 12% 0%,rgba(255,62,207,.30),transparent 30%),radial-gradient(circle at 85% 8%,rgba(0,217,255,.25),transparent 34%),linear-gradient(145deg,#07051b,#071a3a 58%,#180724);padding:18px}.wrap{max-width:1180px;margin:auto}.hero{display:grid;grid-template-columns:1.25fr .75fr;gap:16px;margin-bottom:16px}.card{background:linear-gradient(150deg,var(--card),var(--card2));border:1px solid var(--line);border-radius:22px;padding:18px;box-shadow:0 20px 70px rgba(0,0,0,.28);backdrop-filter:blur(10px)}.title{font-size:2rem;font-weight:900;letter-spacing:-.04em}.brand span{color:var(--pink)}.sub{color:var(--muted);margin-top:6px}.bigStatus{display:flex;align-items:center;gap:12px;margin-top:18px}.orb{width:18px;height:18px;border-radius:50%;background:var(--red);box-shadow:0 0 20px var(--red)}.orb.on{background:var(--green);box-shadow:0 0 24px var(--green)}.orb.warn{background:var(--yellow);box-shadow:0 0 24px var(--yellow)}.statusText{font-size:1.45rem;font-weight:900}.grid{display:grid;grid-template-columns:repeat(12,1fr);gap:16px}.span4{grid-column:span 4}.span6{grid-column:span 6}.span8{grid-column:span 8}.span12{grid-column:span 12}.row{display:flex;justify-content:space-between;gap:14px;border-bottom:1px solid var(--line);padding:10px 0}.row:last-child{border-bottom:0}.label{color:var(--muted)}.value{font-weight:800;color:var(--yellow);word-break:break-word}.badge{display:inline-flex;align-items:center;gap:6px;padding:6px 11px;border-radius:999px;font-weight:900;font-size:.82rem}.on{background:rgba(40,245,164,.16);color:var(--green);border:1px solid rgba(40,245,164,.35)}.off{background:rgba(120,130,160,.18);color:#cbd3e6;border:1px solid rgba(255,255,255,.12)}.error{background:rgba(255,77,125,.16);color:var(--red);border:1px solid rgba(255,77,125,.35)}.warn{background:rgba(255,209,102,.16);color:var(--yellow);border:1px solid rgba(255,209,102,.35)}.info{background:rgba(0,217,255,.14);color:var(--cyan);border:1px solid rgba(0,217,255,.35)}.metric{font-size:3.4rem;font-weight:950;letter-spacing:-.06em;line-height:1}.unit{font-size:1rem;color:var(--muted);font-weight:700}.wallbox{height:210px;display:flex;align-items:center;justify-content:center;position:relative}.box{width:150px;height:170px;border-radius:26px;background:linear-gradient(150deg,rgba(255,255,255,.14),rgba(255,255,255,.04));border:1px solid rgba(255,255,255,.18);box-shadow:inset 0 0 28px rgba(0,217,255,.12),0 0 42px rgba(255,62,207,.12);position:relative;text-align:center;padding-top:22px}.charging .box{animation:pulse 1.2s infinite alternate;box-shadow:0 0 45px rgba(0,217,255,.45),0 0 70px rgba(255,62,207,.28)}@keyframes pulse{from{transform:translateY(0)}to{transform:translateY(-3px)}}.bolt{font-size:54px;color:var(--cyan);text-shadow:0 0 24px var(--cyan)}.cable{position:absolute;bottom:10px;left:50%;width:4px;height:58px;background:linear-gradient(var(--cyan),var(--pink));border-radius:5px}.ev{position:absolute;bottom:0;right:88px;color:var(--muted);font-weight:900}.timeline{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-top:14px}.step{padding:10px;border-radius:14px;background:rgba(255,255,255,.06);border:1px solid var(--line);text-align:center;color:var(--muted);font-weight:800}.step.ok{color:var(--green);border-color:rgba(40,245,164,.35);background:rgba(40,245,164,.09)}.step.active{color:var(--yellow);border-color:rgba(255,209,102,.42);background:rgba(255,209,102,.10)}canvas{width:100%;height:250px;background:rgba(5,8,25,.55);border-radius:18px;border:1px solid var(--line)}details{margin-top:10px;background:rgba(0,0,0,.22);border:1px solid var(--line);border-radius:14px;padding:10px}summary{cursor:pointer;font-weight:900;color:var(--cyan)}pre{white-space:pre-wrap;word-break:break-word;color:#dbe7ff;font-family:ui-monospace,Consolas,monospace;font-size:.78rem}.sessions{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:10px}.session{border:1px solid var(--line);border-radius:16px;padding:12px;background:rgba(255,255,255,.05)}button{border:0;border-radius:10px;padding:8px 11px;margin:4px;cursor:pointer;font-weight:900}.btnPink{background:var(--pink);color:#fff}.btnBlue{background:var(--blue);color:#fff}.btnCyan{background:var(--cyan);color:#001c25}@media(max-width:850px){.hero{grid-template-columns:1fr}.span4,.span6,.span8{grid-column:span 12}.metric{font-size:2.7rem}.timeline{grid-template-columns:1fr 1fr}}
</style></head><body><div class="wrap">
<div class="hero"><div class="card"><div class="brand title">charge<span>cloud</span> Glow Dashboard</div><div class="sub">OCPP 1.6J EVSE Simulator · Europe’s digital heart of EV charging</div><div class="bigStatus"><span id="heroOrb" class="orb"></span><div><div id="heroStatus" class="statusText">Initialisiere…</div><div class="sub"><span id="heroCbid">-</span> · <span id="heroOcpp">-</span></div></div></div><div class="timeline"><div id="tlWifi" class="step">WiFi</div><div id="tlWs" class="step">WebSocket</div><div id="tlBoot" class="step">Boot</div><div id="tlOnline" class="step">Online</div><div id="tlHb" class="step">Heartbeat</div></div></div><div class="card" id="wallCard"><div class="wallbox"><div class="box"><div class="brand"><b>charge<span>cloud</span></b></div><div class="bolt">⚡</div><div id="boxState" class="badge off">-</div><div class="cable"></div></div><div class="ev">EV</div></div></div></div>
<div class="grid"><div class="card span4"><h2>⚡ Ladeleistung</h2><div><span id="powerKw" class="metric">0.0</span> <span class="unit">kW</span></div><div class="row"><span class="label">Strom</span><span><span id="currentA" class="value">0.0</span> A</span></div><div class="row"><span class="label">Status</span><span id="chargerState" class="badge off">-</span></div><div class="row"><span class="label">OCPP Status</span><span id="ocppStatus" class="badge off">-</span></div></div>
<div class="card span4"><h2>🌐 Backend WebSocket</h2><div class="row"><span class="label">Status</span><span id="backendStatus" class="badge off">-</span></div><div class="row"><span class="label">OCPP</span><span id="backendOcppState" class="value">-</span></div><div class="row"><span class="label">Chargebox-ID</span><span id="backendChargeBoxId" class="value">-</span></div><div class="row"><span class="label">URL</span><span id="backendUrl" class="value">-</span></div><div class="row"><span class="label">Transaction</span><span id="backendTxId" class="value">-</span></div></div>
<div class="card span4"><h2>🔌 Connector</h2><div class="row"><span class="label">CP / Connector</span><span><span id="backendCpStatus" class="value">-</span> / <span id="backendConnectorStatus" class="value">-</span></span></div><div class="row"><span class="label">Boot akzeptiert</span><span id="backendBoot" class="value">-</span></div><div class="row"><span class="label">Heartbeat</span><span><span id="backendHeartbeat" class="value">300</span> s</span></div><div class="row"><span class="label">Events</span><span id="backendCount" class="value">0</span></div><div class="row"><span class="label">Letztes Event</span><span id="backendLastEvent" class="value">-</span></div></div>
<div class="card span8"><h2>📈 Ladeleistung Verlauf</h2><canvas id="powerChart" width="900" height="260"></canvas></div>
<div class="card span4"><h2>🚗 Aktuelle Session</h2><div class="row"><span class="label">Session</span><span id="activeState" class="badge off">INAKTIV</span></div><div class="row"><span class="label">Start</span><span id="activeStart" class="value">-</span></div><div class="row"><span class="label">Energie</span><span><span id="activeKwh" class="value">0.000</span> kWh</span></div><div class="row"><span class="label">Kosten</span><span><span id="activeCost" class="value">0.00</span> EUR</span></div></div>
<div class="card span12"><h2>🧪 OCPP Debug</h2><div class="row"><span class="label">Letzte Antwort</span><span id="backendLastMessage" class="value">-</span></div><div class="row"><span class="label">Letzter Fehler</span><span id="backendError" class="value">-</span></div><details><summary>Letzte OCPP TX anzeigen</summary><pre id="backendLastTx">-</pre></details><details><summary>Letzte OCPP RX anzeigen</summary><pre id="backendLastRx">-</pre></details></div>
<div class="card span12"><h2>🧾 Session-Historie</h2><button class="btnCyan" onclick="downloadInvoiceLast3()">Rechnung letzte 3</button><button class="btnPink" onclick="clearSessions()">Alle löschen</button><div id="sessionCards" class="sessions"></div></div></div></div>
<script>
let latest=null,pts=[];const price=5.00;function clsBadge(el,onTxt){el.className='badge '+onTxt}function step(id,ok,active=false){let e=document.getElementById(id);e.className='step '+(ok?'ok':active?'active':'')}function draw(){let c=document.getElementById('powerChart'),x=c.getContext('2d');x.clearRect(0,0,c.width,c.height);x.strokeStyle='rgba(255,255,255,.12)';x.lineWidth=1;for(let i=0;i<5;i++){let y=20+i*52;x.beginPath();x.moveTo(36,y);x.lineTo(880,y);x.stroke()}if(pts.length<2)return;x.strokeStyle='#00d9ff';x.lineWidth=4;x.beginPath();pts.forEach((p,i)=>{let px=40+i*(830/Math.max(1,pts.length-1)),py=235-(p/22)*205;if(i)x.lineTo(px,py);else x.moveTo(px,py)});x.stroke();x.strokeStyle='#ff3ecf';x.lineWidth=1;x.stroke()}async function upd(){let r=await fetch('/api/status');let d=await r.json();latest=d;let b=d.backend||{},charging=d.state=='CHARGING';document.getElementById('heroCbid').textContent=b.chargeBoxId||d.deviceId||'-';document.getElementById('heroOcpp').textContent=b.ocppState||'-';document.getElementById('heroStatus').textContent=charging?'Ladevorgang aktiv':(b.connected?'Backend verbunden':'Backend getrennt');document.getElementById('heroOrb').className='orb '+(charging||b.connected?'on':(b.ocppState=='WIFI_WAIT'?'warn':''));document.getElementById('wallCard').className='card '+(charging?'charging':'');document.getElementById('boxState').textContent=d.state;document.getElementById('boxState').className='badge '+(charging?'info':d.state=='ERROR'?'error':'off');step('tlWifi',d.wifi&&d.wifi.staIp!='0.0.0.0');step('tlWs',b.connected);step('tlBoot',b.bootAccepted,b.ocppState=='BOOT_SENT');step('tlOnline',b.ocppState=='ONLINE');step('tlHb',b.heartbeatSec>0);document.getElementById('chargerState').textContent=d.state;document.getElementById('chargerState').className='badge '+(charging?'info':d.state=='ERROR'?'error':'off');document.getElementById('ocppStatus').textContent=d.ocppStatus||'-';document.getElementById('powerKw').textContent=Number(d.powerKw).toFixed(1);document.getElementById('currentA').textContent=Number(d.currentA).toFixed(1);document.getElementById('backendStatus').textContent=b.connected?'CONNECTED':'DISCONNECTED';document.getElementById('backendStatus').className='badge '+(b.connected?'on':'off');['backendOcppState','backendChargeBoxId','backendUrl','backendTxId','backendCpStatus','backendConnectorStatus','backendHeartbeat','backendCount','backendLastEvent','backendLastMessage','backendError','backendLastTx','backendLastRx'].forEach(id=>{let key=id.replace('backend','');key=key.charAt(0).toLowerCase()+key.slice(1);if(document.getElementById(id))document.getElementById(id).textContent=b[key]??'-'});document.getElementById('backendBoot').textContent=b.bootAccepted?'JA':'NEIN';document.getElementById('activeState').textContent=d.session.active?'AKTIV':'INAKTIV';document.getElementById('activeState').className='badge '+(d.session.active?'on':'off');document.getElementById('activeStart').textContent=d.session.startTime;document.getElementById('activeKwh').textContent=Number(d.session.energyKwh).toFixed(3);document.getElementById('activeCost').textContent=Number(d.session.costEur).toFixed(2);pts.push(Number(d.powerKw));if(pts.length>80)pts.shift();draw();let cards=(d.session.history||[]).slice().reverse().map(s=>`<div class="session"><b>#${s.id}</b><div class="sub">${s.startTime} → ${s.endTime}</div><div><span class="value">${Number(s.energyKwh).toFixed(3)} kWh</span> · ${Number(s.costEur).toFixed(2)} EUR</div><button class="btnBlue" onclick="delSession(${s.id})">Löschen</button></div>`).join('');document.getElementById('sessionCards').innerHTML=cards||'<div class="sub">Noch keine Sessions.</div>'}async function delSession(id){await fetch('/api/session/delete?id='+id);upd()}async function clearSessions(){await fetch('/api/session/clear');upd()}function downloadInvoiceLast3(){if(!latest||!latest.session.history.length){alert('Keine Ladevorgänge vorhanden');return}let arr=latest.session.history.slice(-3),totE=0,totC=0,rows=arr.map(s=>{totE+=Number(s.energyKwh);totC+=Number(s.costEur);return `<tr><td>#${s.id}</td><td>${s.startTime}</td><td>${s.endTime}</td><td>${Number(s.energyKwh).toFixed(3)} kWh</td><td>${price.toFixed(2)} EUR/kWh</td><td>${Number(s.costEur).toFixed(2)} EUR</td></tr>`}).join('');let html=`<html><body><h1>chargecloud Glow Challenge Rechnung</h1><table border="1" cellspacing="0" cellpadding="6"><tr><th>ID</th><th>Start</th><th>Ende</th><th>Verbrauch</th><th>Preis</th><th>Betrag</th></tr>${rows}</table><h2>Gesamt: ${totE.toFixed(3)} kWh / ${totC.toFixed(2)} EUR</h2></body></html>`;let a=document.createElement('a');a.href=URL.createObjectURL(new Blob([html],{type:'text/html'}));a.download='glow-challenge-rechnung-letzte-3.html';a.click()}setInterval(upd,700);upd();
</script></body></html>
)rawliteral"; }
